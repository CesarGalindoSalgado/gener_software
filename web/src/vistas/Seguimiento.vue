      ...
      <table class="w-full text-sm min-w-[640px]">
        <thead>
          <tr class="text-left border-b border-line">
            <th class="px-5 py-2 eyebrow font-normal">Acciones</th>
            ...
            <th class="px-5 py-2 eyebrow font-normal text-center">Días de retraso</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="c in items" :key="c.id" class="border-b border-line last:border-0 hover:bg-secondary/50">
            ...
            <td class="px-5 py-3 text-center">
              <span :class="claseAging(diasDesde(c))" class="font-semibold">{{ diasDesde(c) }}</span>
              <span class="text-muted-ink text-xs"> {{ diasDesde(c) === 1 ? 'día' : 'días' }}</span>
            </td>
          </tr>
        </tbody>
      </table>
